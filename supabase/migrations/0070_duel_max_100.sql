-- Cap Number Duel secrets and guesses at 0–100 so rounds stay tractable
-- (combined with the per-round decimal cap from 0069, the search space stays
-- finite and reachable by binary search).
...
begin
  ...
  if p_secret < 0 or p_secret > 100 then
    raise exception 'Numbers must be between 0 and 100';
  end if;
...
begin
  ...
  if p_value < 0 or p_value > 100 then
    raise exception 'Guesses must be between 0 and 100';
  end if;
  dec := public._duel_decimals(p_round);
