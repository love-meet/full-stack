-- Number Duel progressive difficulty across a 12-round match:
--   rounds 1–6   → Easy   · whole numbers      (0 decimals)
--   rounds 7–10  → Medium · 1 decimal place
--   rounds 11–12 → Hard   · 2 decimal places
-- Enforced server-side so a tampered client can't ignore the cap.

create or replace function public._duel_decimals(p_round int)
returns int
language sql immutable as $$
  select case when p_round <= 6 then 0
              when p_round <= 10 then 1
              else 2 end;
$$;
...
as $$
declare me uuid := auth.uid(); g public.games; r public.duel_rounds; secret_count int; player_count int; dec int;
begin
  if me is null then raise exception 'not authenticated'; end if;
  dec := public._duel_decimals(p_round);
  if p_secret <> round(p_secret, dec) then
    raise exception 'Round % allows numbers with at most % decimal place(s)', p_round, dec;
  end if;
...
language plpgsql security definer set search_path = public
as $$
declare me uuid := auth.uid(); g public.games; r public.duel_rounds; target numeric; fb text; dec int;
begin
  if me is null then raise exception 'not authenticated'; end if;
  dec := public._duel_decimals(p_round);
  if p_value <> round(p_value, dec) then
    raise exception 'Round % allows guesses with at most % decimal place(s)', p_round, dec;
  end if;
  select * into g from public.games where id = p_game_id;
  if g.id is null or g.status <> 'active' or g.current_round <> p_round then raise exception 'round not active'; end if;
  if not exists (select 1 from public.game_players where game_id = p_game_id and user_id = me) then
