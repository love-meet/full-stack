-- Merged from gallery-matching-bots-samuel (was 0097_gallery_matching).
-- Adapted: start_dm removed - see the note where it used to be.
...
-- =========================================================================
-- start_dm is NOT redefined here.
--
-- Samuel's original made it hard-require a mutual match. Victor removed that
-- on 20 Sep: the Message button is the journey, credits are the gate, and a
-- match step would be a second wall in front of the thing we already charge
-- for. The live definition is in 0098 — open, bot-proof, and capped at 20 new
-- conversations a day.
--
-- Everything else in this migration is his, unchanged: the gallery feed, the
-- interest/view tracking, and the mutual-match trigger that fills matches.
-- Those stay as the engine for the serious-relationship path.
