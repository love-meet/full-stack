...
export default function DraughtsScreen() {
  ...
  return (
    <Shell onBack={() => navigate(-1)}>
      <Step>
        <h2 className="text-xl font-extrabold text-gradient-warm">♟ {t('games.draughtsTitle')}</h2>
        {!skipIntro && (
          <>
            <p className="text-sm text-ink-2 mt-1">{t('games.draughtsSubtitle')}</p>
    ...
    <div className="min-h-screen text-ink pb-24">
      ...
      <header className="sticky top-0 z-10 glass border-b border-white/5" style={{ paddingTop: 'var(--lm-top-inset)' }}>
        <div className="max-w-2xl mx-auto h-14 px-3 flex items-center">
          ...
          <div className="flex-1 text-center text-ink font-bold">{t('games.draughtsTitle')}</div>
