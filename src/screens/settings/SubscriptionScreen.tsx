    ...
    <div className="min-h-screen text-ink pb-24">
      ...
      >
        <div className="max-w-2xl mx-auto h-14 px-3 flex items-center">
          ...
          <div className="flex-1 text-center text-ink font-bold">{t('subscription.plans')}</div>
      ...
      <main className="max-w-2xl mx-auto px-5 sm:px-8 py-6 space-y-5">
        <div className="text-center space-y-1">
          <h1 className="text-2xl font-extrabold text-gradient-warm">{t('subscription.headline')}</h1>
        ...
        {active && (
          <div className="glass rounded-3xl p-5 border border-rose/30">
            ...
            <div className="text-sm text-ink-muted">
              {t('subscription.renewsExpires', { date: new Date(active.expires_at).toLocaleDateString() })}
            </div>
          </div>
        )}

        <p className="text-sm text-ink-2">
          {t('subscription.paySecurelyPrefix')} <span className="font-semibold text-ink">ALATPay</span> {t('subscription.paySecuritySuffix')}
        </p>
...
function FreeCard({ isCurrent }: { isCurrent: boolean }) {
  const { t } = useTranslation()
  const features = [t('subscription.freeFeature1'), t('subscription.freeFeature2'), t('subscription.freeFeature3')]
  return (
    <div className={['glass rounded-3xl p-5', isCurrent ? 'ring-1 ring-white/15' : ''].join(' ')}>
      <div className="flex items-baseline justify-between gap-2">
        <h3 className="text-xl font-extrabold text-ink">{t('subscription.free')}</h3>
        <div className="text-right">
          <div className="text-2xl font-extrabold text-ink">$0</div>
          <div className="text-[10px] uppercase tracking-[0.18em] text-ink-muted font-bold">{t('subscription.forever')}</div>
        </div>
      </div>
      <ul className="mt-3 space-y-1">
        ...
        <li className="text-sm text-ink-muted flex items-center gap-2">
          <span>✖</span>
          <span>{t('subscription.cantCreateGames')}</span>
        </li>
        <li className="text-sm text-ink-muted flex items-center gap-2">
          <span>✖</span>
          <span>{t('subscription.cantMessagePremium')}</span>
        </li>
      </ul>
      <div className="mt-4 w-full rounded-full py-2.5 text-sm font-bold text-center bg-surface-3 text-ink-muted">
        {isCurrent ? t('subscription.yourCurrentPlan') : t('subscription.defaultPlan')}
      </div>
    </div>
  )
}
    ...
    >
      <div className="flex items-baseline justify-between gap-2">
        ...
        <div className="text-right">
          ...
          <div className="text-[10px] uppercase tracking-[0.18em] text-ink-muted font-bold">{t('subscription.perMonth')}</div>
        </div>
      </div>
